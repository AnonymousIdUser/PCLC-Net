...
class InterSO3Conv(nn.Module):
    ...
    def forward(self, x, inter_idx=None, inter_w=None):
        # print("xyz:", x.xyz.shape)
        # print("featrue:", x.feats.shape)
...
class PointnetSO3ConvNoPooling(nn.Module):
    '''
    equivariant pointnet architecture for a better extract of spatial point features
    f (nb, nc, np, na) x xyz (nb, 3, np, na) -> (nb, nc, np, na)
    '''
    def __init__(self, dim_in, dim_out, kanchor=60):
        super(PointnetSO3ConvNoPooling, self).__init__()
    ...
    def forward(self, x, pool_a=False):
        ...
        if na == 1:
            feats = torch.cat([x.feats, xyz[...,None]],1) # nb, nc + 3, np, na
        else:
            xyzr = torch.einsum('aji,bjn->bina',self.anchors,xyz)
            feats = torch.cat([x.feats, xyzr],1)
