...
class InvOutBlockOurs(nn.Module):
    def __init__(self, params, norm=None, pooling_method='max', pooling_x=False):
        ...
        # ------------------------------------------------
        if self.pooling_x == True:
            self.pointnet = sptk.PointnetSO3Conv(c_in, c_in, na)
            self.norm.append(nn.BatchNorm1d(c_in))
        else:
            self.pointnet_no_pooling = sptk.PointnetSO3ConvNoPooling(c_in, c_in, na)
            self.norm.append(nn.BatchNorm2d(c_in))
        self.fc2 = nn.Linear(c_in, self.outDim)
    ...
    def forward(self, x, label=None):
        ...
        if self.pooling_x == True:
            x_out = self.pointnet(x_in) # pooling over all points, [nb, nc, np, na]
        else:
            x_out = self.pointnet_no_pooling(x_in)
        ...
        if self.pooling_method == 'mean':
            ...
            # max pooling
            if self.pooling_x == True:
                x_out = x_out.max(2)[0] # [nb, nc, na] -> [nb, nc]
            else:
                x_out = x_out.max(3)[0] # [nb, nc, np, na] -> [nb, nc, np]
        elif self.pooling_method.startswith('attention'):
            out_feat = self.attention_layer(x_out)  # Bx1XA or BxCxA
            confidence = F.softmax(out_feat * self.temperature, dim=2)
            x_out = x_out * confidence
...
class SO3OutBlockR(nn.Module):
    ...
    def forward(self, x, anchors=None):
        ...
        # mean pool at xyz ->  BxCxA
        # print("shape of feature is:", x_out.shape)
        if self.pooling_method == 'mean':
        ...
        # print("shape of feature after pooling is:", x_out.shape)
